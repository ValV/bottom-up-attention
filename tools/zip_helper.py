...
class ZipHelper(object):
    zip_bank = dict()
    non_image_zip_bank = dict()

    def __init__(self):
        super(ZipHelper, self).__init__()

    @staticmethod
    def get_zipfile(path, non_image=False):
        if non_image:
            non_image_zip_bank = ZipHelper.non_image_zip_bank
            if path in non_image_zip_bank:
                return non_image_zip_bank[path]
            else:
                print("creating new non_image_zip_bank")
                zfile = zipfile.ZipFile(path, 'r')
                non_image_zip_bank[path] = zfile
                return non_image_zip_bank[path]
        else:
            zip_bank = ZipHelper.zip_bank
            if path in zip_bank:
    ...
    def list_folder(path):
        zip_path, folder_path = ZipHelper.split_zip_style_path(path)

        zfile = ZipHelper.get_zipfile(zip_path)
        folder_list = []
    ...
    @staticmethod
    def imread(path):
        zip_path, path_img = ZipHelper.split_zip_style_path(path)
        zfile = ZipHelper.get_zipfile(zip_path)
    ...
    def read(path):
        zip_path, path_img = ZipHelper.split_zip_style_path(path)
        zfile = ZipHelper.get_zipfile(zip_path, non_image=False)
        data = zfile.read(path_img)
        return data
